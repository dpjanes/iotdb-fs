 ...
 */
const write_parameterized = (_path, _document, _document_encoding) => {
 ...
 *  Parameterized
 */
const write_json_p = (_path, _json) => _.promise.denodeify((self, done) => {
    _.promise.make(self)
        .then(_.promise.add("path", _path || self.path))
        .then(_.promise.add("json", _.is.Undefined(_json) ? self.json : _json))
        .then(write_json)
...
exports.write.bytes = _.promise.denodeify(write_buffer);
