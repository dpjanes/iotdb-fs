 ...
 *  Remove a file. If the file does not exist, no error will be reported
 */
const remove_directory = _.promise.make((self, done) => {
...
const remove_recursive = _.promise.make((self, done) => {
    ...
    /*
    iotdb_fs.remove = _.promise.make((_self, done) => {
        console.log("REMOVE", _self.path)
        done(null, _self)
    })

    iotdb_fs.remove.directory = _.promise.make((_self, done) => {
        console.log("REMOVE-DIR", _self.path)
        done(null, _self)
    })
    */

    const _handle_folder = _.promise.make((self, done) => {
        _.promise.make(self)
            .then(iotdb_fs.list)
    ...
    const _do = () => {
        if (stack.length === 0) {
            _.promise.make(self)
                .then(_.promise.add("paths", folders))
        ...
        _.promise.make(self)
            .then(_.promise.add("path", first))
            .then(iotdb_fs.is.directory.really)
            .then(_.promise.conditional(sd => sd.exists, _handle_folder))
            .then(_.promise.optional(iotdb_fs.remove))
            .then(_.promise.make(sd => {
