...
/**
 *  Requires: self.path
 *  Accepts: self.filter, self.filter_path, self.otherwise_paths
 *  Produces: self.paths
 */
...
/**
 *  Requires: self.path
 *  Accepts: self.filter, self.filter_path, self.parer, self.parer_path
 *  Produces: self.paths, self.fails
 */
const recursive = (_self, done) => {
    const self = _.d.clone.shallow(_self)
    const method = "lib.list";
 ...
 *  API
 */
exports.list = _.promise.denodeify(list);
exports.list.recursive = _.promise.denodeify(recursive);
